...
import {District, County, PolygonWithHoles, Polygon} from "../src/models/AdministrativeArea";

const isListOfPolygons = (a: any): a is Polygon[] => typeof a[0][0][0] === 'number'

const unifyCoordinates = (coordinates: PolygonWithHoles[] | Polygon[]): PolygonWithHoles[] => {
...
const Home: NextPage = () => {
  ...
  React.useEffect(() => {
    const fetchDb = async () => {
      ...
      setCounties(db.features.map(feature => {
        return {
  ...
  React.useEffect(() => {
    const fetchDb = async () => {
      const response = await window.fetch('/circonscriptions-legislatives.json')
      const db = await response.json()
      ...
      for (const feature of db.features) {
        ...
        lookup[ref].coordinates.push(...unifyCoordinates(feature.geometry.coordinates))
      }
